...
=======
ra_final <- full_join(ra_full, sch_su)
>>>>>>> Stashed changes
...
Approaches: 
1. facet_grid
2. Add another aesthetic to status: color, alpha - doesn't increase readability though
3. Look for another graph
4. merge columns
...
ra_sum3 %>%
  ...
  coord_flip() +
  facet_grid(status~dis_act, labeller = labeller(dis_act = new_labels)) +
...
ra_sum3 %>%
  ggplot(aes(x = fct_rev(lea_state_name), y = tot_st)) +
  geom_col(aes(fill = gender, color = status)) + #, position = "dodge"
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  coord_flip() +
  ...
  labs(x = "State",
       ...
       subtitle = "Visulalized by Gender") +
 # scale_fill_discrete (name = "Gender", labels = c("Female", "Male")) #+
 scale_fill_manual(name = "Gender", values = c("beige","gray"), labels = c("Female", "Male"))
```

```{r}
ra_sum3$group <- paste(ra_sum3$status, ra_sum3$gender, sep="_")
ra_sum3 %>%
  ggplot(aes(x = fct_rev(lea_state_name), y = tot_st)) +
