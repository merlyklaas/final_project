...
# export(susp_k12_tidy, here("data", "susp_k12_tidy.csv"))
# export(susp_ps_tidy, here("data", "susp_ps_tidy.csv"))
...
```{r}
#loading tidy files
#these files are very long and throw errors while knitting
susp_ps_tidy <- import(here("data", "susp_ps_tidy.csv"))
susp_k12_tidy <- import(here("data", "susp_k12_tidy.csv"))
```
...
ggplot(fig_1, aes(fill = `ethnicity/lep/idea`,
           x = type,
           y =sum)) +
...
```{r eval = FALSE}
#these chunks are using a lot of space, and throwing run-time errors. We have saves the images, and are exporting them instead of running these chunks in the knit. 
fig_2 <- susp_k12_tidy %>%
  drop_na() %>%
  ...
  ggplot(fig_2, aes(fill = `ethnicity/lep`,
             x = type,
             y = sum)) +
geom_col(position = "fill",
         stat = "identity",
...
fig_3 <- susp_ps_tidy %>%
  ...
  ggplot(fig_3,aes(`ethnicity/lep/idea`, sum)) +
...
theme(text = element_text(size = 15, face = "bold"),
        axis.text.x = element_text(angle = 90, hjust = 1))
#ggsave(here("plot", "multoos_12.jpg"))
```
