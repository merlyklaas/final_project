...
pacman::p_load(tidyverse, rio, here, ggridges, ggthemes, skimr, tidylog, janitor, reshape2, psych,jtools, lme4, sjPlot, performance,sjmisc) 
...
The rest of this document summarizes methods, results and ends with a disscusion.
...
This analysis was conducted using publicly available data from the 2017-18 civil rights data collection. Each biennium, schools across the United States are required to respond and report key information about their school across a range of topics. This study uses the following school level data in the following areas: 
...
The sample in this study included all K-12 schools across the United States. Overall, the 2017-18 civil rights data collection contains `r count(data_join)` schools who serve students in Grades K-12 during the 2017-18 school year. For research question 1, schools that had missing enrollment information, such as missing counts of total enrollment or enrollment by race/ethnicity, were excluded from the analysis. Moreover, schools that did not report number of suspensions, referrals, arrests, number of novice teachers, were excluded from the analysis. For research question 1, the final sample included `r count(rq1)`` K-12 schools. Table 1 summarizes information on the study sample.
...
## Outcome Measures and Predictor of Interest
...
The main predictor of interest for research question the number of security personnel (sworn law enforcement officers and other security personnel). This variable was calculated by summing the total FTE of sworn law enforcement officers and security personnel to get a count. 
...
For research question 2, the outcome measure of interest was...
...
## Analytic Approach
Descriptive statistics and regression analysis were used to answer the two research questions. To answer the first research question, we used a 3-level hierarchical linear model (HLM) with schools nested within districts and districts nested within states. The final model is represented by the following sets of equations. 
...
To answer the second research question, we used an ordinary least squares regression. 
...
susp_ps_tidy %>% 
  ...
  ggplot(aes(type, students)) + 
  geom_col(aes(fill = `ethnicity/lep/idea`), 
...
```
  * suspension rate by Disability status in k12
```{r}
susp_k12_tidy %>% 
  drop_na() %>% 
  ...
  ggplot(aes(disability, students)) +
  geom_col() + 
  facet_wrap(~`ethnicity/lep`) +
  theme(text = element_text(size = 30, face = "bold"))
ggsave(here("plot", "susp_idea_k12.jpg"))
```

  * Multiple Out of school suspension rate by gender and race in K12
```{r}
susp_k12_tidy %>% 
  drop_na() %>% 
  filter(type == "multoos",
  ...
  mutate(`ethnicity/lep`= factor(`ethnicity/lep`, 
         levels = c("as", "hp", "am", "tr", "wh", "hi", "bl"))) %>% 
  ggplot(aes(`ethnicity/lep`, students)) +
  geom_col(aes(fill = gender),
           position = "dodge") +
  labs(title = "Multiple Out of school suspension rate by gender and race in K12",
       x = "Ethnicity",
