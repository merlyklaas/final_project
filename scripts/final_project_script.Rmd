...
#Distributions of some variables appear to be non-normal. Did you investigate this and might it affect analyses?
```
...
```{r}
susp_ps_tidy %>% 
  filter(`ethnicity/lep/idea` != "lep" | `ethnicity/lep/idea` != "idea" ) %>% 
...
susp_k12_tidy %>% 
  filter(`ethnicity/lep` != "lep") %>% 
   ggplot(aes(type, students)) +
     geom_col(aes(fill = `ethnicity/lep`),
              alpha = 0.7, na.rm = TRUE) 

```

  * Multiple Out of school suspension rate by gender and race
  ###CM Comment - Is anyone else having trouble getting this chunk to run? I get the following error: Error in FUN(X[[i]], ...) : object 'ethnicity/lep/idea' not found
```{r}
susp_k12_tidy %>% 
  filter(type == "multoos") %>% 
  ggplot(aes(`ethnicity/lep/idea`)) +
