...
The following figures provide overviews of the data set at the state level.
...
**Regression analysis** As part of the analysis, conducted 4 models that explored the relaitonship between percent new teachers and rate of suspensions. The first model only included the variable of interest as a control with random intercepts for district and state. The second model added enrollment covariates and school characterstics with random intercept. The third model allowed the relationship between pct new teachers and rate of suspension vary across district. The last model included interactions between enrollment and percent new teachers. Model comparison tests found that the third model was the best model. 
...
This suggests that new teachers may need more support/training so that they are not suspending students. 
...
rq1_m1 <- lmer(susp_std ~ pct_new_teachers + tot_enr + sch_enr_am + sch_enr_as + 
                 sch_enr_bl + sch_enr_hi + sch_enr_hp + sch_enr_tr + 
                 sch_enr_lep + sch_enr_idea + sch_status_sped + 
...
rq1_m2 <- lmer(susp_std ~ pct_new_teachers + tot_enr + sch_enr_am + sch_enr_as + 
                 sch_enr_bl + sch_enr_hi + sch_enr_hp + sch_enr_tr + 
                 sch_enr_lep + sch_enr_idea + sch_status_sped + 
                 sch_status_magnet + sch_status_charter + sch_status_alt + 
                 (pct_new_teachers | leaid) + (1 | lea_state), 
               data = rq1)

#Model with covariates, random intercepts,random slopes and interactions
rq1_m3 <- lmer(susp_std ~ pct_new_teachers + tot_enr + sch_enr_am + sch_enr_as + 
                 sch_enr_bl + sch_enr_hi + sch_enr_hp + sch_enr_tr +  
                 sch_enr_lep + sch_enr_idea + sch_status_sped + 
                 sch_status_magnet + sch_status_charter + sch_status_alt + 
...
tab_model(rq1_m0,rq1_m1,rq1_m2,rq1_m3, collapse.ci = TRUE, p.style = "stars",
          dv.labels = c("Model 1 - No covariates", "Model 2 - Random Intercepts", 
                      "Model 3 - Random Int/Slopes", 
                      "Model 4 - Random Int/Sopes & Interactions"),
...
results <- plot_coefs(rq1_m0,rq1_m1,rq1_m2,rq1_m3, coefs = c(" " = "pct_new_teachers"),
           model.names = c("Model 1 - No covariates", "Model 2 - Random Intercepts", 
                      "Model 3 - Random Int/Slopes", 
                      "Model 4 - Random Int/Sopes & Interactions"),
           legend.title = "Estimate relationship between rate of \n\ novice teachers and rate of suspensions",
           ci.level = .5,
           robust = FALSE,
           point.shape = FALSE)
           
results + labs(x = "Change in number of students suspended per 1000 students \n\ for every percentage point increase in the rate of novice teachers") + xlim(-1,2) + theme(axis.text=element_text(size=16), legend.text=element_text(size = 12))
```
