...
#^^this is a cool way to load packages. thanks for showing me! I'll certainly use in the future. 
#it was neat to see the request to load a package the program knew i needed but didn't have. Great job! 
...
# library(crayon)
```
#fun new packages! i like crayon for adding style to text. Naniar sounds like it is so helpful when working with a big dataset and missing values. I'm curious about how you found these packages, and will ask during your presentation :) 
...
Since we're using multiple files, we created a master file to have all the necessary variables in one data set, called data_joined. We exported this file to reduce the run-time of the code. Below is our data cleaning process #and our creation of the master file#, but `eval = FALSE` as we don't want to run this code each time.

#good use of eval=FALSE to be able to show the code and process
...
#We also had to change a value '-9' to NA and then get rid of NA in our data set and did it in a different way. It's interesting to see how you modified your data set and how you added a function. Great use of mutate and select
```
...
enroll_total <- enroll %>% 
    ...
    group_by(combokey, enroll_type) %>% 
        ...
        ) %>%  #very clever! great combination of functions
    pivot_wider(
        names_from = 2,
        values_from = total,
        names_sep = "_",
...
#good use of pivot_longer and pivot_wider to get the information you want. 
#seems like the first section of code (pivot_longer) makes a clean set by itself that can be used for visualizations etc (with no data in the labels) and then the pivot_wider gets you information you want all together, but makes it messier again (data in the labels). i might be misunderstanding the goal and data though
...
#similar comment to above. great job extracting the information you want for the analysis. the final set seems 'messy' with data in the tables, but that may be how you want it for analysis as far as categories and organizing the data (i know that's the way it is for some data in our set) 
```
...
enroll_final <- enroll_tidy %>% 
    ...
    as_tibble()
```
#3 way join-- cool! i'm learning a lot from your code! 
...
#i like how your clarified this data. an example of how NA actually tells us something when considered in context of the data.
...
```
#good use of select. 
#i'm seeing the benefit of building a function to remove neg bc it can be used throughout!
...
#i'm impressed with the code above! thanks for teaching me about str_remove and str_detect          
```
...
#how will you use this data above?
#it is neat manipulation/analytics, but I don’t see it in results or helping to address a research question, so could get rid of it for overall paper/data clarity
```
...
#you've worked hard to get to this point! can you add a description stating that you will join all the data sets by "combokey" (and define what "combokey" is)
...
#wow such a comprehensive join. now i'm realizing that your use of pivots etc was likely also to help get variables to be similar to join. is that correct? that would be an interesting point to remark on in your paper as you had to tidy and manipulate a lot of data to effectively join all the different sets.
...
#Good use of tabs in descriptive section. I look forward to seeing how each are filled in with summary tables or plots to visualize the different data sets and how you join them together. 
...
#the descriptions/labels of each created data set add helpful clarity. it would be great to see those in the steps above too. 
...
# Methods
#excellent description of your methods. it adds a lot of clarity to your process of exploring and analyzing the data
...
ra_full_c <- ra_full %>%
  clean_names()
```
#I imagine showing these tables may be part of the process to explore the data, so may not be in the final at all. But if they are, Can these tables go in the Appendix instead of the results section? Or could you create a summary table with select variables usingn {gt} or {kable?}, and/or show as a tibble  For me, (admittedly a novice), they are clunky and don’t provide easily digestible information for the reader, and a summary table with defined variables (such as what is SCH_DISCWODIS ?) would be very helpful
#all this said, you are working with SO much data and it is very impressive how you are making sense of it all and using it to answer interesting questions
...
#good use of select to get the info you want!
```
...
#ah here is the definition ive been waiting for! Thank you 😊 
```{r}
ra_sum3 <- ra_sum2 %>%
...
#excellent way to continue to tidy the data after the pivot longer. 
#it's clear you're using the functions to help you get the information you want! 
```
...
```
#terrific! I like how you flipped the axis. Makes it very clear
```{r}
ra_sum3 %>%
  ggplot(aes(x = fct_rev(lea_state_name), y = tot_st)) +
...
#can you use a different color scheme like colorblind to make it more accessible to all readers?
...
#Tables: nice table! Can you change the heading of the columns? Right now they appear the same
```
...
#helpful description of what you are doing
